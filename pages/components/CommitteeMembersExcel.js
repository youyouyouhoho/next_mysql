// CommitteeMembersExcel.js 委员表
import React, { Component } from 'react';
import { Button, Upload, message, Table } from 'antd';
import * as XLSX from 'xlsx';
import { UploadOutlined } from '@ant-design/icons';
class CommitteeMembersExcel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            fileList: [],
    ...
    handleInsertCommitteeMembersToDB = async () => {
        try {
            ...
            for (let rowIndex = 0; rowIndex < this.state.excelData.length; rowIndex++) {
                ...
                const mappedRow = {
                    ...
                    department: row['单位'],
                    title: row['职称'],
                    isSupervisor: row['是否硕导'] === '是' ? true : false,
                    isInSchool: row['校内or校外'] === '校内' ? false : true,
                };

                const response = await fetch('/api/insertCommitteeMembers', {
                    method: 'POST',
                    headers: {
            ...
            if (successCount === this.state.excelData.length) {
                ...
                try {
                    const response = await fetch('/api/delete?table=committeemembers', { method: 'DELETE' });
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.state.excelData.length > 0 && <Button style={{ marginTop: '20px' }} onClick={this.handleInsertCommitteeMembersToDB}>
                    将委员数据插入数据库
                </Button>}
...
export default CommitteeMembersExcel;
