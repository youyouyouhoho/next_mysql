// ReviewInfoExcel.js 评阅场次信息表
import React, { Component } from 'react';
import { Button, Upload, message, Table } from 'antd';
import * as XLSX from 'xlsx';
import { UploadOutlined } from '@ant-design/icons';
class ReviewInfoExcel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            fileList: [],
    ...
    handleInsertReviewInfoToDB = async () => {
        try {
            ...
            for (let rowIndex = 0; rowIndex < this.state.excelData.length; rowIndex++) {
                ...
                const mappedRow = {
                    ...
                    reviewTimeStart: this.parseDefenseTime(row['答辩时间'])[0],
                    reviewTimeEnd: this.parseDefenseTime(row['答辩时间'])[1],
                    defenseLocation: row['答辩地点'],
                    committeeMember1: row['评阅委员1'],
                    committeeMember2: row['评阅委员2'],
                    committeeMember3: row['评阅委员3'],
                };

                // Execute the API request to insert data into the database
                const response = await fetch('/api/insertReviewInfo', {
                    method: 'POST',
                    headers: {
            ...
            if (successCount === this.state.excelData.length) {
                ...
                try {
                    const response = await fetch('/api/delete?table=reviewinfo', { method: 'DELETE' });
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.state.excelData.length > 0 && <Button style={{ marginTop: '20px' }} onClick={this.handleInsertReviewInfoToDB}>
                    将评阅场次信息插入数据库
                </Button>}
...
export default ReviewInfoExcel;
