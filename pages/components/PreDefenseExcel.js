// PreDefenseExcel.js 预答辩场次信息表
import React, { Component } from 'react';
import { Button, Upload, message, Table } from 'antd';
import * as XLSX from 'xlsx';
import { UploadOutlined } from '@ant-design/icons';
class PreDefenseExcel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            fileList: [],
    ...
    handleInsertPreDefenseToDB = async () => {
        try {
            ...
            for (let rowIndex = 0; rowIndex < this.state.excelData.length; rowIndex++) {
                ...
                const mappedRow = {
                    ...
                    qualificationNumber: row['资格证号'],
                    // Handle pre-defense time string conversion
                    preDefenseTimeStart: this.parseDefenseTime(row['答辩时间'])[0],
                    preDefenseTimeEnd: this.parseDefenseTime(row['答辩时间'])[1],
                    defenseLocation: row['答辩地点'],
                    committeeMember1: row['预答辩委员1'],
                    committeeMember2: row['预答辩委员2'],
                    committeeMember3: row['预答辩委员3'],
                };
    
                // Execute the SQL insert query for the 'PreDefenseInfo' table
                const response = await fetch('/api/insertPreDefenseInfo', {
                    method: 'POST',
                    headers: {
            ...
            if (successCount === this.state.excelData.length) {
                ...
                try {
                    const response = await fetch('/api/delete?table=predefenseinfo', { method: 'DELETE' });
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.state.excelData.length > 0 && <Button style={{ marginTop: '20px' }} onClick={this.handleInsertPreDefenseToDB}>
                    将预答辩场次信息插入数据库
                </Button>}
...
export default PreDefenseExcel;
